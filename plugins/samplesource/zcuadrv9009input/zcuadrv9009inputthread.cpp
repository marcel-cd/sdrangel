...
Zcuadrv9009InputThread::Zcuadrv9009InputThread(uint32_t blocksizeSamples, DeviceZcuadrv9009Box* plutoBox, SampleSinkFifo* sampleFifo, QObject* parent) :
    QThread(parent),
    m_running(false),
    m_plutoBox(plutoBox),
    ...
    m_log2Decim(0),
    m_fcPos(Zcuadrv9009InputSettings::FC_POS_CENTER),
    m_phasor(0),
    m_iqOrder(true)
{
...
Zcuadrv9009InputThread::~Zcuadrv9009InputThread()
{
    stopWork();
    delete[] m_buf;
}

void Zcuadrv9009InputThread::startWork()
{
...
void Zcuadrv9009InputThread::stopWork()
{
...
void Zcuadrv9009InputThread::setLog2Decimation(unsigned int log2_decim)
{
    m_log2Decim = log2_decim;
}

void Zcuadrv9009InputThread::setFcPos(int fcPos)
{
    m_fcPos = fcPos;
}

void Zcuadrv9009InputThread::run()
{
    std::ptrdiff_t p_inc = m_plutoBox->rxBufferStep();

    qDebug("Zcuadrv9009InputThread::run: rxBufferStep: %ld bytes", p_inc);
    qDebug("Zcuadrv9009InputThread::run: Rx sample size is %ld bytes", m_plutoBox->getRxSampleSize());
    qDebug("Zcuadrv9009InputThread::run: Tx sample size is %ld bytes", m_plutoBox->getTxSampleSize());
    qDebug("Zcuadrv9009InputThread::run: nominal nbytes_rx is %d bytes with 1 refill", m_blockSizeSamples*4);
    ...
    while (m_running)
    {
        ...
        {
            qWarning("Zcuadrv9009InputThread::run: error refilling buf %d / %d",(int) nbytes_rx, (int)  m_blockSizeSamples*4);
            usleep(200000);
            continue;
        }
...
void Zcuadrv9009InputThread::convertIQ(const qint16* buf, qint32 len)
{
...
void Zcuadrv9009InputThread::convertQI(const qint16* buf, qint32 len)
{
    SampleVector::iterator it = m_convertBuffer.begin();
