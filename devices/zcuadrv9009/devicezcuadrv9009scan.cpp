...
#include "devicezcuadrv9009scan.h"

void DeviceZcuadrv9009Scan::scan()
{
    int i, num_contexts;
    ...
    if (!scan_ctx)
    {
        qCritical("Zcuadrv9009Scan::scan: could not create scan context");
        return;
    }

    num_contexts = iio_scan_context_get_info_list(scan_ctx, &info);
    qDebug("Zcuadrv9009Scan::scan: num_contexts %d", num_contexts);

    if (num_contexts < 0)
    {
        qCritical("Zcuadrv9009Scan::scan: could not get contexts");
        return;
    }
    ...
    for (i = 0; i < num_contexts; i++)
    {
        ...
        if (!DeviceZcuadrv9009Box::probeURI(std::string(uri))) { // continue if not accessible
            continue;
        }

        qDebug("Zcuadrv9009Scan::scan: %d: %s [%s]", i, description, uri);
        char *pch = strstr(const_cast<char*>(description), "Zcuadrv9009");
...
const std::string* DeviceZcuadrv9009Scan::getURIAt(unsigned int index) const
{
...
const std::string* DeviceZcuadrv9009Scan::getSerialAt(unsigned int index) const
{
    if (index < m_scans.size()) {
...
const std::string* DeviceZcuadrv9009Scan::getURIFromSerial(
...
void DeviceZcuadrv9009Scan::getSerials(std::vector<std::string>& serials) const
{
...
void DeviceZcuadrv9009Scan::enumOriginDevices(const QString& hardwareId, PluginInterface::OriginDevices& originDevices)
{
    scan();
	...
	for (i = 0; it != serials.end(); ++it, ++i)
	{
	    ...
	    QString displayableName(QString("Zcuadrv9009[%1] %2").arg(i).arg(serial_str));
        ...
        qDebug("DeviceZcuadrv9009Scan::enumOriginDevices: enumerated Zcuadrv9009 device #%d", i);
	}
